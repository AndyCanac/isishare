...
export default function Skill() {
  ...
  useEffect(() => {
    ...
    fetch(
      `http://localhost:3001/skills?id=${localStorage.getItem("idTargetUser")}`
    )
      .then((response) => {
        if (!response.ok) {
                ...
                <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                  <thead className="bg-gray-50 dark:bg-gray-800">
                    <tr>
                      ...
                      <th
                        scope="col"
                        className="bg-dark-blue px-4 py-3.5 text-sm font-normal text-left rtl:text-right text-white dark:text-gray-400 w-2/5"
                      >
                        Description
                      </th>
                      <th
                      ...
                      <tr key={skill}>
                        <td className="px-4 py-4 text-sm text-gray-500 dark:text-gray-300">
                          <img
                            ...
                            src={skill.iconInteret}
                            alt="logo"
                          />
                        </td>
                        <td className="px-4 py-4 text-sm text-gray-500 dark:text-gray-300">
                          {skill.niveauConnaissance}/5
                        </td>
                        <td className="px-4 py-4 text-sm text-gray-500 dark:text-gray-300">
                          {skill.descriptionConnaissance}
                        </td>
                        <td className="px-4 py-4 text-sm text-gray-500 dark:text-gray-300">
                          {skill.lienConnaissance}
                        </td>
                        <td className="px-4 py-4 text-sm">
                          <div className="flex items-center gap-x-6">
                            {ownUser ? (
                              <button
                                onClick={() =>
                                  deleteConnaissanceTrigger(
                                    skill.idConnaissance
                                  )
                                }
