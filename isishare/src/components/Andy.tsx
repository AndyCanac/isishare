...
export default function Andy(){
    ...
    const [showDeleteContact, setShowDelete] = useState(false);
      ...
      .catch((error) => {
        console.error("Error fetching users:", error);
          ...
          const fetchNotations = fetch(`${localStorage.getItem("api")}notations`)
            ...
            Promise.all([fetchSources, fetchNotations, fetchUser, fetchUserContact, fetchSkills, fetchInterets]).then(() => {
              setIsLoading(false);
            });
        }
...
//#region WTL
  ...
  const handleInteretChangeWTL = (event: React.ChangeEvent<HTMLSelectElement>) => {
    setInteret(event.target.value as any);
  };

  const handleDescriptionChangeWTL = (
    event: React.ChangeEvent<HTMLInputElement>
  ) => {
...
//#endregion
 ...
 <div className="min-h-screen bg-gray-100 flex flex-col lg:flex-row p-6">
      ...
      <div className="flex flex-col w-full lg:w-[calc(100%-4rem)] pr-[150px]">
          ...
          <div className="flex justify-between items-center mb-4">
            ...
            <table className="min-w-full divide-y divide-gray-200">
                ...
                <tr>
                    ...
                    <th className="px-6 py-3 bg-gray-50 text-left text-xs font-medium text-white uppercase tracking-wider">{ownUser ? "Action" : ""}</th>
        ...
        <div className="bg-white rounded-lg shadow-lg p-6 overflow-x-auto">
          ...
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-dark-blue">
              <tr>
                <th className="px-6 py-3 bg-gray-50 text-left text-xs font-medium text-white uppercase tracking-wider">Connaissance</th>
                <th className="px-6 py-3 bg-gray-50 text-white">Description</th>
                {ownUser ? (
                <th className="px-6 py-3 bg-gray-50 text-white">Action</th>
                ) : null}
              </tr>
            </thead>
                <tbody className="bg-white divide-y divide-gray-200">
