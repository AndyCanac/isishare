...
export default function SignUp() {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!apiUrl) {
        throw new Error("L'URL de l'API n'est pas définie dans le stockage local.");
      }
      // hash password
      ...
      const url = `${apiUrl}users/insert/name,email,password/"${encodeURIComponent(name)}","${encodeURIComponent(email)}","${encodeURIComponent(hashedPassword)}"`;
      ...
      if (!response.ok) {
        ...
        throw new Error(`Erreur de réseau: ${response.status} ${response.statusText} - ${errorText}`);
      }
  ...
  return (
    <div className="mx-auto max-w-screen-xl px-4 py-16 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-lg">
        <a href="/">
          <Image src="/isishare.png" className="w-full max-w-lg lg:mx-auto" width={300} height={300} alt="Logo" />
        </a>
